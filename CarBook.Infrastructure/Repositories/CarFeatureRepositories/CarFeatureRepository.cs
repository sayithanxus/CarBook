...
namespace CarBook.Persistence.Repositories.CarFeatureRepositories
{
    public class CarFeatureRepository : ICarFeatureRepository
    {
        ...
        public List<Feature> GetFeaturesNotInCar(int carID)
        {
            var carFeatures = GetCarFeaturesByCarID(carID)
             .Select(cf => cf.FeatureID)
             .ToList();

            // Arabada olmayan özellikler
            var featuresNotInCar = _context.Features
                .Where(f => !carFeatures.Contains(f.FeatureID))
                .ToList();
